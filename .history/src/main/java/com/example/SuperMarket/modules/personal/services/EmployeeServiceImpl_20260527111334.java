...
public class EmployeeServiceImpl implements EmployeeService {
    ...
    public List<EmployeeResponseDTO> getEmployeesByRole(String role) {
        try{
            ...
            throw new RuntimeException("El cargo '" + role + "'no es valido");
        }
    }

    @Transactional(readOnly = true)
    public List<EmployeeResponseDTO> getEmployeesByHiredDateRange(LocalDate startDate, LocalDate endDate){
        if (startDate.isAfter(endDate)){
            throw new RuntimeException("La fecha de inicio de jornada no puede ser posterior a la fecha de fin de jornada");
        }

        return employeeRepository.findByfechaContratacionBetween(startDate, endDate).stream()
                .map(this::mapToResponseDTO)
                .collect(Collectors.toList());
    }

    private EmployeeResponseDTO mapToResponseDTO(Employee empleado){
