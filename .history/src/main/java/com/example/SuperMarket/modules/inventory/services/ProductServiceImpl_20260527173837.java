...
public class ProductServiceImpl implements ProductService {
    ...
    @Override
    @Transactional(readOnly = true)
    public ProductResponseDTO getProductByCodigoBarras(String barcode) {
        Product product = productRepository.findByBarcodeAndActiveTrue(barcode)
                .orElseThrow(() -> new RuntimeException("No se encontró ningún producto activo con el código: " + barcode));
        return mapToResponseDTO(product);
    }
    ...
    public ProductResponseDTO updateProduct(Long id, ProductRequestDTO requestDTO) {
        ...
        if (!product.getBarcode().equals(requestDTO.getBarcode()) && productRepository.existsByBarcode(requestDTO.getBarcode())) {
            throw new RuntimeException("El código de barras '" + requestDTO.getBarcode() + "' ya pertenece a otro producto.");
        }
